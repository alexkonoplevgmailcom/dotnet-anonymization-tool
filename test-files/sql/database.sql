...
CREATE TABLE AcmeCustomers (
    CustomerID INT IDENTITY(1,1) PRIMARY KEY, -- Auto-incrementing ACME customer ID
    FirstName NVARCHAR(50) NOT NULL,         -- Customer first name
    LastName NVARCHAR(50) NOT NULL,          -- Customer last name
    Email NVARCHAR(100) NOT NULL UNIQUE,     -- Email address (must be unique in ACME system)
    Phone NVARCHAR(20),                      -- Phone number
    CreatedDate DATETIME DEFAULT GETDATE(),  -- When customer was added to ACME system
    LastUpdated DATETIME DEFAULT GETDATE(),  -- Last modification date
    IsActive BIT DEFAULT 1,                  -- Status in ACME system
    
    -- ACME-specific fields
    AcmeMembershipLevel NVARCHAR(20) DEFAULT 'Bronze', -- ACME membership tier
    AcmeEmployeeID NVARCHAR(50),                        -- If customer is ACME employee
    AcmeRegion NVARCHAR(50) DEFAULT 'North America'     -- ACME operational region
);
...
CREATE TABLE AcmeOrders (
    OrderID INT IDENTITY(1,1) PRIMARY KEY,   -- Unique ACME order identifier
    CustomerID INT NOT NULL,                 -- Reference to ACME customer
    OrderDate DATETIME DEFAULT GETDATE(),   -- When order was placed in ACME system
    OrderTotal DECIMAL(10,2) NOT NULL,      -- Total order amount
    OrderStatus NVARCHAR(20) DEFAULT 'Pending', -- Current status in ACME workflow
    
    -- ACME shipping information
    ShippingAddress NVARCHAR(200),          -- Delivery address for ACME shipments
    AcmeShippingMethod NVARCHAR(50),        -- ACME shipping option selected
    AcmeTrackingNumber NVARCHAR(100),       -- ACME tracking identifier
...
CREATE TABLE AcmeProducts (
    ProductID INT IDENTITY(1,1) PRIMARY KEY, -- ACME product identifier
    ProductName NVARCHAR(100) NOT NULL,     -- Product name in ACME catalog
    ProductDescription NTEXT,               -- Detailed product description
    Price DECIMAL(10,2) NOT NULL,          -- Current price in ACME system
    CategoryID INT,                         -- ACME product category
    
    -- ACME inventory fields
    StockQuantity INT DEFAULT 0,            -- Available stock in ACME warehouses
    AcmeSupplierID NVARCHAR(50),           -- ACME supplier reference
    IsAcmeExclusive BIT DEFAULT 0          -- Whether product is ACME exclusive
);
...
CREATE PROCEDURE sp_GetAcmeCustomerSummary
    @CustomerID INT -- ACME customer identifier
AS
BEGIN
    ...
    SELECT 
        ...
        ISNULL(SUM(o.OrderTotal), 0) AS TotalSpent -- Total spent with ACME
    FROM AcmeCustomers c
    LEFT JOIN AcmeOrders o ON c.CustomerID = o.CustomerID
    WHERE c.CustomerID = @CustomerID
      AND c.IsActive = 1 -- Only active ACME customers
...
CREATE PROCEDURE sp_UpdateAcmeMembershipLevel
    @CustomerID INT,        -- ACME customer to update
    @NewLevel NVARCHAR(20)  -- New ACME membership level
AS
BEGIN
    /*
    ...
    WHERE CustomerID = @CustomerID
      AND IsActive = 1; -- Only update active ACME customers
    
    -- Log the change in ACME audit trail
    INSERT INTO AcmeAuditLog (CustomerID, Action, Details, ActionDate)
    VALUES (@CustomerID, 'Membership Update', 
            'Changed to ' + @NewLevel, GETDATE());
END;

-- Create ACME audit log table
CREATE TABLE AcmeAuditLog (
    LogID INT IDENTITY(1,1) PRIMARY KEY,    -- ACME audit log identifier
    CustomerID INT,                         -- Related ACME customer
    Action NVARCHAR(50) NOT NULL,          -- Action performed in ACME system
    Details NVARCHAR(200),                 -- Additional details
    ActionDate DATETIME DEFAULT GETDATE(), -- When action occurred
    PerformedBy NVARCHAR(100)              -- ACME user who performed action
);
...
INSERT INTO AcmeCustomers (FirstName, LastName, Email, AcmeMembershipLevel, AcmeRegion)
VALUES 
...
INSERT INTO AcmeProducts (ProductName, ProductDescription, Price, IsAcmeExclusive)
VALUES 
...
-- Sample ACME orders
INSERT INTO AcmeOrders (CustomerID, OrderTotal, AcmeShippingMethod)
VALUES 
